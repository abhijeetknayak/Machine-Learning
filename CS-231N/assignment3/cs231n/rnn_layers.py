...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, prev_h, z, Wx, Wh = cache
    dlocal = 1 / (np.cosh(z) ** 2)  # Local gradient intermediate is sech() squared
    
    dlocal = dnext_h * dlocal
    
    db = np.sum(dlocal, axis=0)
    
    dWx = np.dot(x.T, dlocal)
    dWh = np.dot(prev_h.T, dlocal)
    
    dprev_h = np.dot(dlocal, Wh.T)
    dx = np.dot(dlocal, Wx.T)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    # Initialize h and prev_c with zeros
    h = np.zeros((N, T, H))   
    prev_c = np.zeros((N, H))
